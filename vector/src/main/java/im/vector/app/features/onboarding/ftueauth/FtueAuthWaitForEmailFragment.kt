...
class FtueAuthWaitForEmailFragment :
    ...
    private fun setupUi() {
        views.waitForEmailGradientContainer.setBackgroundResource(themeProvider.ftueBreakerBackground())
        views.waitForEmailTitle.text = getString(CommonStrings.ftue_auth_email_verification_title)
                .colorTerminatingFullStop(ThemeUtils.getColor(requireContext(), com.google.android.material.R.attr.colorSecondary))
        views.waitForEmailSubtitle.text = getString(CommonStrings.ftue_auth_email_verification_subtitle, params.email)
        views.waitForEmailResendEmail.debouncedClicks {
